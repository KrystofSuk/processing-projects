/* autogenerated by Processing revision 1276 on 2021-10-06 */
...
public class sketch_211006 extends PApplet {
...
int beatDur = 6;
 ...
 public void setup() {
    ...
    for (int i = 0; i < 17; ++i) {
        print(i + "\n");
        if(imgs[i].width *.7f > width && imgs[i].height *.7f > height)
            imgs[i].resize(round(imgs[i].width *.7f), round(imgs[i].height *.7f));
...
int phaseDur = 0;
 ...
 public void draw() {
    ...
    if(cnt <= 0){
        cnt = 120;
        dX = random(-0.07f, 0.06f);
        dY = random(-0.07f, 0.06f);
    }
    if(cnt2 <= 0){
        cnt2 = 120*2;
    ...
    dtX = lerp(dtX, dX, 0.001f);
    dtY = lerp(dtY, dY, 0.001f);
    dtT = lerp(dtT, dT, 0.01f);
    ...
    if (bt == 0) {
        perspective(PI / 3.0f, width / height, 10, 10000000);
        
        cam.rotateY(dtX + 0.01f);
        cam.rotateX(dtY + 0.01f);
        cam.setDistance(0 + abs(sin(frameCount * 0.005f)) * 600);
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "sketch_211006" };
    if (passedArgs != null) {
